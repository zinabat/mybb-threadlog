...
$plugins->add_hook("admin_config_plugins_begin", "threadlog_regen_missing_entries");
function threadlog_regen_missing_entries() {
    global $db, $mybb;
    ...
    $query = $db->write_query("SELECT distinct p.uid,p.tid from `{$db->table_prefix}posts` p
        where (p.uid,p.tid) not in
            (select uid,tid from `{$db->table_prefix}threadlogentry`)");
    $inserts = [];
    while ($row = $db->fetch_array($query)) {
        if (!(int) $row['uid']) continue; // ignore uid=0
    ...
    if (!empty($inserts)) {
        $db->insert_query_multiple('threadlogentry', $inserts);
        // don't worry about order, just stick them first
        flash_message("Missing threadlogs regenerated", "success");
    }
