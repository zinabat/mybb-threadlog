...
class Threadlog {
    ...
    */
    ...
    private function handle_reorder_multi() {
        ...
        // change the keys for the page
        $perpage = (int) $mybb->settings['threadlog_perpage'];
        $adjusted_entries = [];
        $base = ($page - 1) * $perpage;
        foreach ($entries as $i => $entry) {
            $adjusted_entries[$base + $i] = $entry;
        }

        $update_values = [];
        // stitch them into a query with the new given roworder
        foreach ($adjusted_entries as $i => $entry) {
    ...
    private function handle_reorder_single() {
        ...
        if ($direction !== 'up' && $direction !== 'down') {
            self::json_response(403, "Direction must be up or down, got '{$direction}'");
        }
        ...
        $query = $db->write_query(self::generate_select_query($mybb->user['uid']) . " and roworder ".($direction === 'up' ? '<' : '>')."
        ...
        $current_roworder = $direction === 'up' ? (int) $entry_to_swap['roworder'] + 1 : (int) $entry_to_swap['roworder'] - 1;
        ...
        if ($mybb->input['template']) {
            ...
            $template_row = self::row_template_values($entry_to_swap,
    ...
    public static function generate_select_query($uid = null) {
        global $mybb, $db;
